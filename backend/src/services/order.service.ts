...
export class OrderService {
  ...
  private static fallbackOrders: any[] = [];
  private static fallbackNextId = 1;
  async createOrder(
    userId: number,
  ...
  async getAdminStats() {
    try {
      ...
      const totalSalesRes = await (prisma as any).order.aggregate({
        _sum: { total: true },
      });
      ...
      const totalUsers = await prisma.user.count();
      const totalSales = OrderService.fallbackOrders.reduce(
        (s, o) => s + (o.total || 0),
        0
      );
      return { totalOrders, totalUsers, totalSales };
  ...
  async getAllOrders(options?: {
    ...
    try {
      ...
      if (userId) arr = arr.filter((o) => o.userId === userId);
      if (status) arr = arr.filter((o) => o.status === status);
      if (from) arr = arr.filter((o) => new Date(o.createdAt) >= from);
      if (to) arr = arr.filter((o) => new Date(o.createdAt) <= to);
      arr.sort(
        (a, b) =>
  ...
  async getSalesByRange(from: Date, to: Date) {
    try {
      const orders = await (prisma as any).order.findMany({
